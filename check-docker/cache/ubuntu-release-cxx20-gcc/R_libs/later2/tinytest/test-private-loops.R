...
local({
  ...
  # effect immediately.
  expect_true(later2:::notify_r_ref_deleted(l1))
...
local({
  ...
  # Empty the queue by calling run now. Also test calling later2:::list_queue by passing
  # in a loop handle.
  with_loop(l, later2::run_now())
  q <- later2:::list_queue(l)
  expect_equal(length(q), 0)

  destroy_loop(l)
  expect_error(later2:::list_queue(l))
})
...
local({
  ...
  expect_error(next_op_secs(loop))
})
...
local({
  ...
  expect_error(with_loop(loop, destroy_loop(loop)))
  expect_true(destroy_loop(loop))
  expect_false(destroy_loop(loop))
  expect_error(with_loop(loop, {}))
  expect_error(loop_empty(loop))
  expect_error(create_loop(parent = "invalid"))
})

local({
  # print.event_loop works correctly ----
...
local({
  ...
  loop <- create_loop(parent = NULL)
  expect_error(later2:::notify_r_ref_deleted(global_loop()))
  expect_error(with_loop(loop, later2:::notify_r_ref_deleted(loop)))
  expect_error({
    with_loop(loop, {
